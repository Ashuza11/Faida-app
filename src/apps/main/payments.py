...
def allocate_registered_client_payment(
    *, client_id: int, vendeur_id: int, business_id: int | None,
    ...
    Ad-hoc customers are deliberately unsupported here: a display name is not
    an identity and must never cause money to move between unrelated people.
    """
    ...
    unpaid_sales = query.order_by(Sale.sale_date.asc(), Sale.created_at.asc()).all()
...
def apply_payment_to_sale(*, sale: Sale, amount: Decimal, recorded_by, payment_date: date) -> Decimal:
    ...
    if sale.client_id is not None:
        ...
        maximum += old_debt
    if amount > maximum:
        raise ValueError("Le montant payé dépasse la dette totale du client et la vente actuelle.")
